...
SWEP.PrintName = "Franchi SPAS-12(Short)"
...
SWEP.WepSelectIcon2 = Material("vgui/spas12s.png")
SWEP.IconOverride = "vgui/spas12s.png"
...
SWEP.holsteredPos = Vector(-16.6, 8, 0)
...
SWEP.AnimList = {
	...
	["start_empty"] = "sgreload_start_empty_semi",
	["cycle"] = "cycle",
}

SWEP.AnimsEvents = {
	["sgreload_start_empty_semi"] = {
		[0.2] = function(self)
			self:EmitSound("weapons/arccw_ur/spas12/breechload.ogg")
			--self:GetWM():ManipulateBoneScale(47, vector_full)
		end,
		[0.4] = function(self)
			self:EmitSound("weapons/arccw_ur/spas12/breechclose.ogg")
		end,
	},
	["sgreload_insert"] = {
		[0.0] = function(self)
...
local function reloadFunc(self)
	...
	self:PlayAnim(self.AnimList["insert"] or "sgreload_insert", 0.65, false, function() 
		...
		if !self.drawBullet then
			...
			self:PlayAnim(self.AnimList["finish_empty"] or "sgreload_finish_empty", 0.65,false,function(self)  
...
function SWEP:Reload(time)
	...
	if SERVER then
		self:SetNetVar("shootgunReload",CurTime() + (self:Clip1() == 0 and 2.1 or 0.75))
		
		local anim = self:Clip1() == 0 and "start_empty" or "start"
		self:PlayAnim(self.AnimList[anim] or "sgreload_start",self:Clip1() == 0 and 2 or 0.65,false,function() 
			self:SetNetVar("shootgunReload",CurTime() + 0.75)
