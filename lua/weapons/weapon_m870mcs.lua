...
SWEP.PrintName = "Remington M870 MCS"
...
SWEP.WepSelectIcon2 = Material("vgui/m870mcs.png")
SWEP.IconOverride = "vgui/m870mcs.png"
...
SWEP.holsteredPos = Vector(-15.6, -1, -3)
...
local function reloadFunc(self)
	...
	self:SetNetVar("shootgunReload",CurTime() + 0.75)

	if self.MagIndex then
		self:GetWM():ManipulateBoneScale(self.MagIndex, vector_full)
	end
	
	self:PlayAnim(self.AnimList["insert"] or "sgreload_insert", 0.65, false, function() 
		self:InsertAmmo(1) 
		if self.MagIndex then
		...
		if !self.drawBullet then
			cock(self,1)
			self:PlayAnim(self.AnimList["finish_empty"] or "sgreload_finish_empty", 0.65, false, function(self) self:SetNetVar("shootgunReload", 0) end, false, true) 
		else
			self:PlayAnim(self.AnimList["finish"] or "sgreload_finish", 0.65, false, function(self) self:SetNetVar("shootgunReload", 0) end, false, true) 
		end
	end, false, true)
end
...
function SWEP:Reload(time)
	...
	if self.drawBullet == false and SERVER then
		cock(self,1)
		self:SetNetVar("shootgunReload",CurTime() + 0.75)
		self:PlayAnim(self.AnimList["cycle"] or "cycle", 0.65, false, nil, false, true)
		return
	end
	...
	if SERVER then
		...
		self:PlayAnim(self.AnimList["start"] or "sgreload_start",0.65,false,function() 
			reloadFunc(self)
		end,
		false, true)
	end
