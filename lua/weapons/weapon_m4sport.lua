...
SWEP.PrintName = "Benelli M4 Super 90 Sport"
...
SWEP.WepSelectIcon2 = Material("vgui/m4supers.png")
SWEP.IconOverride = "vgui/m4supers.png"
...
SWEP.HoldType = "rpg"
SWEP.ZoomPos = Vector(-3, 0.3264, 2.3564)
...
SWEP.AnimList = {
	...
	["finish_empty"] = "sgreload_finish_stock",
	["start_empty"] = "sgreload_start_empty_stock",
	["finish"] = "sgreload_finish_stock",
	["insert"] = "sgreload_insert",
	["start"] = "sgreload_start_stock",
	["cycle"] = "cycle",
}
...
local function reloadFunc(self)
	...
	self:SetNetVar("shootgunReload",CurTime() + 0.85)

	self:GetWM():ManipulateBoneScale(47, vector_full)
	--self:GetOwner():PullLHTowards("ValveBiped.Bip01_Spine2", 0.58)

	self:PlayAnim(self.AnimList["insert"] or "sgreload_insert", 0.75, false, function() 
		self:InsertAmmo(1) 
		self:GetWM():ManipulateBoneScale(47, vector_origin)
		...
		self:PlayAnim(self.AnimList["finish"] or "sgreload_finish", 0.75,false,function(self) self:SetNetVar("shootgunReload",0) end,false,true) 
	end, false, true)
end
...
function SWEP:Reload(time)
	...
	--self:GetWM():ManipulateBoneScale(47, vector_full)
	if SERVER then
		self:SetNetVar("shootgunReload",CurTime() + (self:Clip1() == 0 and 1.6 or 0.85))
		
		local anim = self:Clip1() == 0 and "start_empty" or "start"
		self:PlayAnim(self.AnimList[anim] or "sgreload_start",self:Clip1() == 0 and 1.5 or 0.75,false,function() 
			self:SetNetVar("shootgunReload",CurTime() + 0.85)
