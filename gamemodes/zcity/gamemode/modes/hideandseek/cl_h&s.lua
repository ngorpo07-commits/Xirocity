...
local teams = {
    [0] = { //свата нету
        objective = "ОБЕЗВРЕДЬ ЕГО",
        name = "полицейский",
        color1 = Color(68, 10, 255),
        color2 = Color(68, 10, 255)
    },
    [1] = {
        objective = "У некоторых людей злые намерения. Спрячься от них, они скоро прибудут.",
        name = "Жертва",
        color1 = Color(0, 190, 190),
        color2 = Color(0, 190, 190)
    },
    [2] = {
        objective = "Дороги назад нету, убей их всех...",
        name = "Шутер",
...
function MODE:HUDPaint()
    ...
    if zb.ROUND_START + 60 > CurTime() then
        ...
        draw.SimpleText( "Шутер прибудет через: "..string.FormattedTime(zb.ROUND_START + 60 - CurTime(), "%02i:%02i"), "ZB_HomicideMedium", sw * 0.02 + posadd, sh * 0.91, Color(0,0,0), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
        draw.SimpleText( "Шутер прибудет через: "..string.FormattedTime(zb.ROUND_START + 60 - CurTime(), "%02i:%02i"), "ZB_HomicideMedium", (sw * 0.02) - 2 + posadd, (sh * 0.91) - 2, blink, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
    end
    if zb.ROUND_START + 240 > CurTime() then
        posadd = Lerp(FrameTime() * 5,posadd or 0, zb.ROUND_START + 7.3 < CurTime() and 0 or -sw * 0.4) 
        local color = Color(255*-math.sin(CurTime()*3),25,255*math.sin(CurTime()*3))
        draw.SimpleText( string.FormattedTime(zb.ROUND_START + 240 - CurTime(), "%02i:%02i").." До окончания стрельбы", "ZB_HomicideMedium", sw * 0.02 + posadd, sh * 0.95, Color(0,0,0), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
        draw.SimpleText( string.FormattedTime(zb.ROUND_START + 240 - CurTime(), "%02i:%02i").." До окончания стрельбы", "ZB_HomicideMedium", (sw * 0.02) - 2 + posadd, (sh * 0.95) - 2, color, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
        local fade = math.Clamp(zb.ROUND_START + 7.5 - CurTime(), 0, 1)
        surface.SetDrawColor(0, 0, 0, 255 * fade)
        surface.DrawRect(-1, -1, ScrW() + 1, ScrH() + 1)
    end
